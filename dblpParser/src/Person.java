...
public class Person {
    ...
    static public void printCoauthorTable2() {
    ...
    static public void printCoauthorTable() {
        Iterator it = Person.iterator();
        Person pers;
        StringBuilder coauthorTable = new StringBuilder();
        while (it.hasNext()) {
            pers = (Person) it.next();
            coauthorTable.append(pers.name);
            Person coauthors[] = pers.getCoauthors();
            for(int i=0; i<coauthors.length; i++) {
            	Person coauthor = coauthors[i];
            	coauthorTable.append( ";" + coauthor.name);
            }
            coauthorTable.append("\n");
        }
        System.out.println(coauthorTable);
    }
